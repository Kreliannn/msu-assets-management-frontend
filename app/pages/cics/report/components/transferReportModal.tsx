...
export function TransferReportModal({ open, onOpenChange }: TransferReportModalProps) {
  ...
  const load = async () => {
    ...
    setError("")
    try {
      const res = await axiosInstance.get("/system/transfer-requests")
      const data = res.data as transferRequestInterface[]
      setRows(
        data
          .filter((t) => t.college === "CICS")
          .map((t) => ({
            assetname: t.assetname,
            college: t.college || "Main Office",
            custodian: t.custodian || "Main Office",
            date: t.date,
            status: t.status,
          }))
      )
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] flex flex-col p-0 gap-0 overflow-hidden">
        <div className="p-4 border-b flex items-center justify-between gap-4 shrink-0">
          <DialogHeader className="p-0">
            <DialogTitle>Transfer Report</DialogTitle>
          </DialogHeader>
          <Button
        ...
        <div className="flex-1 overflow-auto p-4 sm:p-8 bg-slate-100">
          <ReportPaper title="Transfer Report">
